...
class ReshapeWeightForTiling(OptimizerPass):
    ...
    def match(self, node):
        node_matches = isinstance(node, (MultiheadAttention, FeedForwardNetwork))
    ...
    def transform(self, model, node):
        ...
        if isinstance(node, (MultiheadAttention)): 
            node.weights['in_proj_weight'].data = np.reshape(node.weights['in_proj_weight'].data, \
                                                            ...
                                                            node.weights['in_proj_weight'].data.shape[0]//3//node.get_attr("tiling_factor")[2]//node.get_attr("num_heads"), \
            ...
            node.weights['in_proj_weight'].data = np.transpose(node.weights['in_proj_weight'].data, axes=[4, 2, 0, 1, 5, 3]).flatten()
            ...
            node.weights['in_proj_bias'].data = np.reshape(node.weights['in_proj_bias'].data, \
                                                            (3, node.get_attr("num_heads"), node.weights['in_proj_bias'].data.shape[0]//3//node.get_attr("tiling_factor")[2]//node.get_attr("num_heads"), node.get_attr("tiling_factor")[2]))
            node.weights['in_proj_bias'].data = np.transpose(node.weights['in_proj_bias'].data, axes=[2, 0, 1, 3]).flatten()
            ...
            node.weights['out_proj_weight'].data = np.reshape(node.weights['out_proj_weight'].data, \
                                                            ...
                                                            node.get_attr("tiling_factor")[1], \
                                                            node.get_attr("num_heads"), \
                                                            node.weights['out_proj_weight'].data.shape[1]//node.get_attr("tiling_factor")[2]//node.get_attr("num_heads"), \
                                                            node.get_attr("tiling_factor")[2]))
            #node.weights['out_proj_weight'].data = np.transpose(node.weights['out_proj_weight'].data, axes=[2, 3, 0, 4, 1])
            node.weights['out_proj_weight'].data = np.transpose(node.weights['out_proj_weight'].data, axes=[3, 0, 2, 4, 1]).flatten()
            ...
            node.weights['out_proj_bias'].data = node.weights['out_proj_bias'].data.flatten()
            node.weights['out_proj_bias'].shape = node.weights['out_proj_bias'].data.shape
            node.weights['mask'].data = np.reshape(node.weights['mask'].data, (node.weights['mask'].data.shape[0]//node.get_attr("tiling_factor")[0], \
                                                                               node.get_attr("tiling_factor")[0], \
                                                                                node.weights['mask'].data.shape[1]//node.get_attr("tiling_factor")[0], \
                                                                                node.get_attr("tiling_factor")[0]))
            node.weights['mask'].data = np.transpose(node.weights['mask'].data, axes=[0, 2, 1, 3]).flatten()
            node.weights['mask'].shape = node.weights['mask'].data.shape
            ...
            node.weights['in_proj_weight'].data = np.reshape(node.weights['in_proj_weight'].data, \
                                                            ...
                                                            node.weights['in_proj_weight'].data.shape[1]//node.get_attr("tiling_factor")[1], \
                                                            node.get_attr("tiling_factor")[1]))
            node.weights['in_proj_weight'].data = np.transpose(node.weights['in_proj_weight'].data, axes=[2, 0, 3, 1]).flatten()
            node.weights['out_proj_weight'].data = np.reshape(node.weights['out_proj_weight'].data,  \
                                                            (node.weights['out_proj_weight'].data.shape[0]//node.get_attr("tiling_factor")[1], \
                                                            node.get_attr("tiling_factor")[1], \
                                                            node.weights['out_proj_weight'].data.shape[1]//node.get_attr("tiling_factor")[2], \
                                                            node.get_attr("tiling_factor")[2]))
            node.weights['out_proj_weight'].data = np.transpose(node.weights['out_proj_weight'].data, axes=[2, 0, 3, 1]).flatten()  
            node.weights['in_proj_weight'].shape = node.weights['in_proj_weight'].data.shape
            node.weights['out_proj_weight'].shape = node.weights['out_proj_weight'].data.shape
            #node.weights['in_proj_bias'].data = np.reshape(node.weights['in_proj_bias'].data, (node.weights['in_proj_bias'].data.shape[0]//node.get_attr("tiling_factor")[2], node.get_attr("tiling_factor")[2]))
            #node.weights['in_proj_bias'].shape = node.weights['in_proj_bias'].data.shape
            #node.weights['out_proj_bias'].data = np.reshape(node.weights['out_proj_bias'].data, (node.weights['out_proj_bias'].data.shape[0]//node.get_attr("tiling_factor")[1], node.get_attr("tiling_factor")[1]))
            #node.weights['out_proj_bias'].shape = node.weights['out_proj_bias'].data.shape
