            ...
            if (k==0) {
                ...
                for (int i = 0; i < CONFIG_T::n_head; i++) {//8dsp
                    ...
                    for (int ii = 0; ii < CONFIG_T::tiling_factor[0]; ii++) {
                        ...
                        prev_exp_tmp[i][ii] = lookup_exp<typename CONFIG_T::accum_t, CONFIG_T>(prev_rowmax[i][ii] - new_rowmax[i][ii]);
                        exp_tmp[i][ii] = lookup_exp<typename CONFIG_T::accum_t, CONFIG_T>(rowmax[i][ii] - new_rowmax[i][ii]);
